...
//The number of projects a user was a part of that are now closed
if(isset($_GET['id'])){
	$accountId = $_GET['id'];
	...
	if($userProfile){
		...
		if($accountId == $user->getAccountId() || !$isPrivate || ($isPrivate && $user->hasFriend($accountId))){
			...
			$numProjects = ProjectModel::getCountOfDiscontinuedProjectsForAccount($accountId);	
			...
			//The projects a user was a part of that are now closed
			$projects = ProjectModel::getDiscontinuedProjectsForAccount($accountId, $currentPageNum, 6);
		} else{?>
			<script type="text/javascript">
...
<div class = 'filter-page'>
	<div class='learn-more-title filter-title'>
		...
		<h2>which have been discontined</h2>
		<hr>
		<p>
			<?php if(count($projects) > 0){?>
			<i>Below are projects that <?php echo $firstname?> was a part of which have been <b>discontinued</b>.  You can also view projects 
			<?php }else{?>
			<i><?php echo $firstname;?> has not been involved in any projects which have been discontinued. You can see if there are projects
			<?php }?>				
						<?php echo $firstname;?> was a part of which are 
						<a href='#' onclick='PageChanger.loadPersonalProjectsView({id: <?php echo $accountId?>})'>active</a>
						 or have been 
						 <a href='#' onclick="PageChanger.loadPersonalCompletedProjectsView({id: <?php echo $accountId;?>})">completed</a>.</i></p>
	</div>
	<div class='projects-viewer-container'>
		...
		<div class='nav-row'>
			...
			<?php if($numProjects > 6){?>
				<a href="#" class='small-nav-button first' data-role='button' onclick="PageChanger.loadAchieveStageProjectsView({'page' :'1', 'id' :'<?php echo $accountId;?>'})">First</a>
				<div class='small-spacing' style='display:inline-block;width:20px'></div>
				<a href="#" class='big-nav-button previous' data-role='button' onclick="PageChanger.loadAchieveStageProjectsView({'page' :'<?php echo $currentPageNum-1?>', 'id' :'<?php echo $accountId;?>'})">Previous</a>
				<div class='big-spacing' style='display:inline-block;width:50px'></div>
				<a href="#" class='big-nav-button next' data-role='button' onclick="PageChanger.loadAchieveStageProjectsView({'page' :'<?php echo $currentPageNum+1?>', 'id' :'<?php echo $accountId;?>'})">Next</a>
				<div class='small-spacing' style='display:inline-block;width:20px'></div>
				<a href="#" class='small-nav-button last' data-role='button' onclick="PageChanger.loadAchieveStageProjectsView({'page' :'<?php echo $numPages?>', 'id' :'<?php echo $accountId;?>'})">Last</a>	
			</span>
